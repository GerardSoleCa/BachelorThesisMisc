...
#define GPIO_PATH new String("/sys/class/gpio/")
namespace Microsoft {
	namespace SPOT {
		namespace Manager {
...
//				Console::WriteLine((int)type);
				...
				if (GPIOManager::_activePins->ContainsKey(pin)) {
//					Console::WriteLine("Contains key ok");
...
//						Console::WriteLine("GPIOMANAGER::SetPortType OUTPUT");
					...
					case PortType::INTERRUPT:
						Console::WriteLine("I'm interrupt pin");
						direction = new String("in");
...
//					Console::WriteLine(tmp);
				...
				throw new Exception(new String("ola ke ase"));
			}
			...
			void GPIOManager::Listen(Object* obj)
			{
				...
				while (true) {
					...
					DELEGATE_INVOKE(th->getCallback(), (uint)(cback->pin), (uint)0, null);
				}
			}
