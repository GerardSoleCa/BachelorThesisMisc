...
\\
The origin of this project resides in the need of migrating the code of an existing Wireless Sensor Network (\gls{WSN}) gateway implemented on Micro Framework for a Netduino Mini board. One of the problems that this platform has is that the actual device is getting out of system resources, so in order to keep the existing code, different solutions are being researched. In this case IOSharp intends to be one of those solutions achieving the deployment of this WSN gateway software on a Raspberry Pi which runs a Linux operating system.
\\
\\
